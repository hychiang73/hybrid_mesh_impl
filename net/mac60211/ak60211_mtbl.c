...
static int ak60211_sta_info_hash_add(struct ak60211_if_data *local,
				     struct ak60211_sta_info *sta)
{
	return rhltable_insert(&local->sta_hash,
			       &sta->hash_node,
...
static void ak60211_sta_info_free(struct ak60211_if_data *local,
				  struct ak60211_sta_info *sta)
{
...
static void ak60211_mpath_free_rcu(struct ak60211_mesh_table *tbl,
...
static struct ak60211_sta_info *__ak60211_mesh_sta_alloc(struct ak60211_if_data *ifmsh,
							 u8 *addr)
{
	struct ak60211_sta_info *sta;
...
static struct ak60211_mesh_path *ak60211_mpath_new(struct ak60211_if_data *ifmsh,
						   const u8 *dst, gfp_t gfp)
{
					    ...
					    const u8 *dst)
{
	...
	spin_lock_bh(&tbl->walk_lock);
	mpath = rhashtable_lookup_get_insert_fast(&tbl->rhead, &new_mpath->rhash,
						  ak60211_mesh_rht_params);
	if (!mpath)
