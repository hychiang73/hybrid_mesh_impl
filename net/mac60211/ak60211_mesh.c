...
static int __must_check __ak60211_sta_info_destory(struct ak60211_sta_info *sta, struct ak60211_if_data *ifmsh)
{
    int ret;
    ...
    ret = rhltable_remove(&ifmsh->sta_hash, &sta->hash_node, ak60211_sta_rht_params);
    if (WARN_ON(ret)) {
        return ret;
    }
...
static void ak60211_sta_expire(struct ak60211_if_data *ifmsh, unsigned long exp_time)
{
    ...
    list_for_each_entry_safe(sta, tmp, &ifmsh->sta_list, list) {
        ...
        if (last_active) {
            if (time_is_before_jiffies(last_active + exp_time)) {
                ...
                WARN_ON(__ak60211_sta_info_destory(sta, ifmsh));
            }
        }
    }
...
void ak60211_mesh_plink_frame_tx(struct ak60211_if_data *ifmsh, enum ieee80211_self_protected_actioncode action, 
...
static void ak60211_mesh_bcn_presp(struct plc_packet_union *buff, struct ak60211_if_data *ifmsh)
{
    ...
    if (stype == AK60211_STYPE_PROBE_RESP && memcmp(buff->plchdr.machdr.h_addr3, ifmsh->addr, ETH_ALEN)) {
        return;
    }
...
int ak60211_rx_handler(struct sk_buff *pskb)
{
    ...
    if ((!!memcmp(plcbuff->da, plcdev.addr, ETH_ALEN)) && !is_broadcast_ether_addr(plcbuff->da)) {
        goto drop;
    }
...
static void plc_gen_sbeacon(struct plc_packet_union *buff)
{
    ...
    memcpy(buff->un.beacon.meshid_elem.meshid, meshid, MAX_MESH_ID_LEN);       // max meshid size copy
...
{
    struct ak60211_if_data *ifmsh = from_timer(ifmsh, t, housekeeping_timer);
...
bool ak60211_mesh_init(u8 *id, u8 *mac)
{
    ...
    memcpy(&plcdev.mshcfg, &default_mesh_config, sizeof(struct ak60211_mesh_config));
    ...
    timer_setup(&plcdev.housekeeping_timer, ak60211_mesh_housekeeping_timer, 0);
    set_bit(MESH_WORK_HOUSEKEEPING, &plcdev.wrkq_flags);
