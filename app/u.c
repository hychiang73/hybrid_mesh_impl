...
*/
    ...
    //msg->nl_msghdr.nlmsg_len = NLMSG_SPACE(buf_len) + sizeof(msg->if_index);
    msg->nl_msghdr.nlmsg_len = sizeof(struct nl60211msg) - MAX_PAYLOAD + buf_len;
...
int do_debug(int argc, char **argv)
{
    ...
    if_nl_send(NL60211_DEBUG, if_idx, sizeof(req->len)+/*da,sa,ether_type,payload*/argc);
...
int do_setmeshid(int argc, char **argv)
{
    ...
    printf("req->id_len = %d\n", req->id_len);
    printf("req->id = %s\n", req->id);
    if_nl_send(NL60211_SETMESHID, if_idx, sizeof(struct nl60211_setmeshid_req) + req->id_len + 1/* '\0' for teh end of C string */);
...
int do_sendplc(int argc, char **argv)
{
    ...
    if_nl_send(NL60211_SEND_PLC, if_idx, sizeof(req->total_len)+/*da,sa,ether_type,payload*/argc);
...
int do_sendwifi(int argc, char **argv)
{
    ...
    if_nl_send(NL60211_SEND_WIFI, if_idx, sizeof(req->total_len)+/*da,sa,ether_type,payload*/argc);
...
int do_floodsned(int argc, char **argv)
{
    ...
    if_nl_send(NL60211_SEND_FLOOD, if_idx, sizeof(req->total_len)+/*da,sa,ether_type,payload*/argc);
...
int do_sendbest(int argc, char **argv)
{
    ...
    if_nl_send(NL60211_SEND_BEST, if_idx, sizeof(req->total_len)+/*da,sa,ether_type,payload*/argc);
...
int main(int argc, char **argv)
{
    ...
    if (argc < 3) {
        fprintf(stderr, "Too few arguments, exit... (a.out [command] [interface])\n");
        return -1;
    }
